...
use crate::{
    ...
    col_toolbox::{
        ...
        final_join_one_to_many::utils::{calc_final_join_one_to_many_index_transform, calc_index_transformed_col},
    },
};

pub struct FinalJoinOneToManyIOP<F:PrimeField, PCS: PCS<F>>(PhantomData<F>, PhantomData<PCS>);

impl <F:PrimeField, PCS: PCS<F>> FinalJoinOneToManyIOP<F, PCS> 
where PCS: PCS<F> {
    pub fn prove(
        prover: &mut ProverTrackerRef<F, PCS>,
        table_a: &Table<F, PCS>, // primary key table, no duplicates
        table_b: &Table<F, PCS>, // foreign key table, has duplicates
        ...
        let mut res_table_a_mles = Vec::<MLE<F>>::with_capacity(table_a.col_vals.len());
        for a_col in table_a.col_vals.iter() {
            res_table_a_mles.push(calc_index_transformed_col(&a_index_transform, a_col, &table_b.selector)?);
        }

        // put res_table_a_mles into the prover tracker
        let mut transformed_a_cols = Vec::<TrackedPoly<F, PCS>>::with_capacity(table_a.col_vals.len());
        for col_mle in res_table_a_mles.iter() {
            transformed_a_cols.push(prover.track_and_commit_poly(col_mle.clone())?);
        }
    ...
    pub fn prove_with_advice(
        ...
        a_join_col_index: usize,
        b_join_col_index: usize,
        transformed_a_cols: &Vec::<TrackedPoly<F, PCS>>,
    ) -> Result<Table<F, PCS>, PolyIOPErrors> {
        ...
        #[cfg(debug_assertions)] 
        {
            ...
            let mut counts = HashMap::<F, usize>::new();
        ...
        prover.add_zerocheck_claim(equality_check_poly.id);
    ...
    pub fn verify_with_advice(
        ...
        a_join_col_index: usize,
        b_join_col_index: usize,
        transformed_a_cols: &Vec::<TrackedOracle<F, PCS>>,
    ) -> Result<TableComm<F, PCS>, PolyIOPErrors> {
        // set up the result table
        let mut res_table_col_polys = transformed_a_cols.clone();
        res_table_col_polys.append(&mut table_b.col_vals.clone());
        ...
        IndexTransformIOP::<F, PCS>::verify(
            verifier,
            table_a,
            &transformed_a_table,
        )?;
        ...
        let join_col_a = res_table.col_vals[a_join_col_index].clone();
        let join_col_b = res_table.col_vals[table_a.col_vals.len() + b_join_col_index].clone();
        let equality_check_poly = join_col_a.sub_oracles(&join_col_b).mul_oracles(&res_table.selector.clone());
        verifier.add_zerocheck_claim(equality_check_poly.id);
