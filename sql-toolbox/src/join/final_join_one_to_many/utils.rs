...
pub fn calc_final_join_one_to_many_index_transform<F, PCS>(
    ...
    PCS: PCS<F>,
{
    ...
    let mut a_join_col_hashmap: HashMap<F, usize> = HashMap::new();
    ...
    for i in 0..a_join_col_evals.len() {
        if a_sel_evals[i] == F::one() { // only include activated rows
    ...
    for i in 0..table_b_col_len {
        if b_sel_evals[i] == F::one() { // only include activated rows
...
pub fn calc_index_transformed_col<F, PCS>(
    index_transform: &Vec<usize>,
    old_poly: &TrackedPoly<F, PCS>,
    new_activator: &TrackedPoly<F, PCS>,
) -> Result<MLE<F>, PolyIOPErrors>
where
    E: Pairing,
    PCS: PCS<F>,
{
    ...
    let mut transformed_poly_evals = Vec::<F>::with_capacity(new_len);
    for i in 0..new_len {
        if new_activator_evals[i] == F::one() {
            transformed_poly_evals.push(old_poly_evals[index_transform[i]]);
        } else {
            transformed_poly_evals.push(F::zero());
        }
    }

    let new_mle = MLE::from_evaluations_vec(new_activator.num_vars(), transformed_poly_evals);
