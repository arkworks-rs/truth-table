...
where
    MvPCS:,
{
    let col_nv = col.log_size();
    let col_len = 2_usize.pow(col_nv as u32);
    let col_poly_evals = col.data_tracked_poly().evaluations();
    let col_sel_evals = col.activator_tracked_poly().as_ref().unwrap().evaluations();
    ...
    let supp_mle = MLE::from_evaluations_vec(col_nv, supp_evals);
    let supp_sel_mle = MLE::from_evaluations_vec(col_nv, supp_sel_evals);
    let multiplicity_mle = MLE::from_evaluations_vec(col_nv, multiplicities);
