...
use ark_piop::{prover::ArgProver, verifier::ArgVerifier, DefaultSnarkBackend, SnarkBackend};
use datafusion::arrow::datatypes::{DataType, Field, Schema};
use indexmap::IndexMap;

use super::{GadgetNode, COL_LABEL, SUPP_LABEL};
...
fn evals_from_u64(values: &[u64]) -> Vec<<Backend as SnarkBackend>::F> {
    values.iter().copied().map(<Backend as SnarkBackend>::F::from).collect()
}
...
fn run_supp_roundtrip(
    col_values: Vec<<Backend as SnarkBackend>::F>,
    col_activator: Option<Vec<<Backend as SnarkBackend>::F>>,
    supp_values: Vec<<Backend as SnarkBackend>::F>,
    supp_activator: Option<Vec<<Backend as SnarkBackend>::F>>,
) -> SnarkResult<()> {
    let (mut prover, mut verifier) = test_prelude::<Backend>().unwrap();
    let col_table = build_tracked_table(&mut prover, "col", col_values, col_activator);
    let supp_table = build_tracked_table(&mut prover, "supp", supp_values, supp_activator);
    ...
    gadget_payload.insert(COL_LABEL.to_string(), col_table.clone());
    gadget_payload.insert(SUPP_LABEL.to_string(), supp_table.clone());
    ...
    let gadget_initialization_pass =
        ProverGadgetInitializationPass::<Backend>::new(gadget_ir_view);
    ...
    let col_oracle = tracked_table_to_oracle(&col_table, &mut verifier);
    let supp_oracle = tracked_table_to_oracle(&supp_table, &mut verifier);

    let mut gadget_payload = IndexMap::new();
    gadget_payload.insert(COL_LABEL.to_string(), col_oracle);
    gadget_payload.insert(SUPP_LABEL.to_string(), supp_oracle);
...
#[test]
fn gadget_supp_completeness_no_activator() {
    let col_values = evals_from_u64(&[25, 7, 7, 7]);
    let supp_values = evals_from_u64(&[25, 7]);
    run_supp_roundtrip(col_values, None, supp_values, None).unwrap();
}

#[test]
fn gadget_supp_completeness_with_activator() {
    let col_values = evals_from_u64(&[7, 2, 3, 3, 2, 6, 6, 7]);
    let supp_values = evals_from_u64(&[7, 6, 6, 5, 1, 2, 2, 3]);
    let supp_activator = evals_from_u64(&[1, 1, 0, 0, 0, 0, 1, 1]);
    run_supp_roundtrip(col_values, None, supp_values, Some(supp_activator)).unwrap();
}

#[test]
fn gadget_supp_soundness_rejects_mismatch() {
    let col_values = evals_from_u64(&[25, 7, 9, 2]);
    let supp_values = evals_from_u64(&[25, 9, 6, 2]);
    let err = run_supp_roundtrip(col_values, None, supp_values, None).unwrap_err();
