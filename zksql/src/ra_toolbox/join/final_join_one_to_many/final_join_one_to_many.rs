...
pub struct FinalJoinOneToManyIOP<F:PrimeField, PCS: PolynomialCommitmentScheme<F>>(PhantomData<F>, PhantomData<PCS>);

impl <F:PrimeField, PCS: PolynomialCommitmentScheme<F>> FinalJoinOneToManyIOP<F, PCS> 
where PCS: PolynomialCommitmentScheme<F> {
    pub fn prove(
        ...
        let mut res_table_a_mles = Vec::<DenseMultilinearExtension<F>>::with_capacity(table_a.col_vals.len());
    ...
    pub fn verify(
        ...
        let mut transformed_a_cols = Vec::<TrackedComm<F, PCS>>::with_capacity(table_a.col_vals.len());
    ...
    pub fn verify_with_advice(
        ...
        a_join_col_index: usize,
        b_join_col_index: usize,
        transformed_a_cols: &Vec::<TrackedComm<F, PCS>>,
    ) -> Result<TableComm<F, PCS>, PolyIOPErrors> {
        // set up the result table
        let mut res_table_col_polys = transformed_a_cols.clone();
        res_table_col_polys.append(&mut table_b.col_vals.clone());
