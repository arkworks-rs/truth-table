...
mod test {
    ...
    use crate::{
        tracker::prelude::*,
        ra_piop::group_by::group_by::{AggregationType, GroupByIOP, GroupByInstructionWithProvingAdvice, GroupByInstructionWithVerifyingAdvice},
    ...
    fn test_group_by_sum() -> Result<(), PolyIOPErrors> {
        ...
        let bad_result = test_group_by_helper::<<ark_ec::bls12::Bls12<ark_test_curves::bls12_381::Config> as ark_ec::pairing::Pairing>::ScalarField, MultilinearKzgPCS::<Bls12_381>>(
            &mut prover_tracker, 
            &mut verifier_tracker, 
            &table_vals, 
            &table_sel, 
    ...
    fn test_group_by_helper<F, PCS>(
        prover_tracker: &mut ProverTrackerRef<F, PCS>,
        verifier_tracker: &mut VerifierTrackerRef<F, PCS>,
        table_vals: &Vec<DenseMultilinearExtension<F>>,
        table_sel: &DenseMultilinearExtension<F>,
        grouping_cols: &Vec<usize>,
        support_cols: &Vec<DenseMultilinearExtension<F>>,
        support_sel: &DenseMultilinearExtension<F>,
        support_mult: &DenseMultilinearExtension<F>,
        agg_mle_instructions: &Vec<(usize, AggregationType, DenseMultilinearExtension<F>)>,
        range_mle: &DenseMultilinearExtension<F>,
        range_sel_mle: &DenseMultilinearExtension<F>,
        range_nv: usize,
    ...
    PCS: PolynomialCommitmentScheme<F>,
    {
        ...
        let mut prover_agg_instructions: Vec<(usize, AggregationType, TrackedPoly<F, PCS>)> = Vec::new();
        ...
        let range_col = Col::new(range_poly.clone(), range_sel_poly.clone());
        ...
        let mut verifier_agg_instructions: Vec<(usize, AggregationType, TrackedComm<F, PCS>)> = Vec::new();
        ...
        let range_col_comm = ColComm::new(verifier_tracker.transfer_prover_comm(range_poly.id), verifier_tracker.transfer_prover_comm(range_sel_poly.id), range_nv);
