...
// use crate::col_toolbox::cross_product::utils::{
...
pub struct CrossProductIOP<F:PrimeField, PCS: PolynomialCommitmentScheme<F>>(PhantomData<F>, PhantomData<PCS>);
impl <F:PrimeField, PCS: PolynomialCommitmentScheme<F>> CrossProductIOP<F, PCS> 
where PCS: PolynomialCommitmentScheme<F> {
    pub fn prover_cross_product(
        ...
        let mut result_table_col_vals = Vec::<TrackedPoly<F, PCS>>::with_capacity(result_cols_len);
    ...
    pub fn verifier_cross_product(
        table_a: &TableComm<F, PCS>,
        table_b: &TableComm<F, PCS>,
    ) -> Result<TableComm<F, PCS>, PolyIOPErrors> {
        let table_a_nv = table_a.num_vars();
        let table_b_nv = table_b.num_vars();
        let result_nv = table_a_nv + table_b_nv;
        let result_cols_len = table_a.col_vals.len() + table_b.col_vals.len();
        let mut result_table_col_vals = Vec::<TrackedComm<F, PCS>>::with_capacity(result_cols_len);
