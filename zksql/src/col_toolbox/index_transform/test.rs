...
mod test {
    ...
    use crate::{
        tracker::prelude::*,
        col_toolbox::index_transform::index_transform::IndexTransformIOP,
    };
    ...
    fn test_index_transform_helper<F, PCS>(
        prover_tracker: &mut ProverTrackerRef<F, PCS>,
        verifier_tracker: &mut VerifierTrackerRef<F, PCS>,
        table_in_sel_mle: &DenseMultilinearExtension<F>,
        table_in_cols_mle: &[DenseMultilinearExtension<F>],
        table_out_sel_mle: &DenseMultilinearExtension<F>,
        table_out_cols_mle: &[DenseMultilinearExtension<F>],
    ) -> Result<(), PolyIOPErrors>
    where
    PCS: PolynomialCommitmentScheme<F>,
    {
        let table_in_sel = prover_tracker.track_and_commit_poly(table_in_sel_mle.clone())?;
        let mut table_in_cols = Vec::<TrackedPoly<F, PCS>>::new();
        ...
        let mut table_out_cols = Vec::<TrackedPoly<F, PCS>>::new();
        ...
        let mut table_in_cols_comm = Vec::<TrackedComm<F, PCS>>::new();
        ...
        let mut table_out_cols_comm = Vec::<TrackedComm<F, PCS>>::new();
