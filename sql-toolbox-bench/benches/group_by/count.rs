...
// use ark_piop::{errors::SnarkResult, piop::PIOP, prover::Prover, verifier::Verifier};
// use sql_toolbox::group_by::{
...
//         Ok::<_, anyhow::Error>((prover, verifier, input_table, output_table, prover_input))
//     })
//     .unwrap()
// }
...
// fn prepare_verifier_inputs() -> (Verifier<F, P, K>, GroupByVerifierInput<F, P, K>) {
//     let (mut prover, mut verifier, input_table, output_table, prover_input) =
//         prepare_prover_inputs();
//     let rt = Runtime::new().unwrap();
//     rt.block_on(async {
//         // Generate proof
//         let _ = GroupByPIOP::<F, P, K>::prove(&mut prover, prover_input.clone());
...
// // seperate proving phase is done. It's bad and gives sigkill. Change the code
// // to do the proving once and clone the verifier for each verification
...
//             let _: SnarkResult<()> = GroupByPIOP::<F, P, K>::verify(&mut verifier, verifier_input);
