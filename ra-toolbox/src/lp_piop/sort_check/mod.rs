...
fn perm_prove<F: PrimeField, MvPCS: PCS<F, Poly = MLE<F>>, UvPCS: PCS<F, Poly = LDE<F>>>(
    prover: &mut Prover<F, MvPCS, UvPCS>,
    ...
    input_key_components.extend(
        input
            .input_sort_exprs
            .iter()
            .map(|tracked| tracked.expr.clone()),
    );
    let mut input_key_col = linear_combine_tracked_cols(input_key_components, &key_fold_challenges);
    input_key_col =
        apply_activator_to_tracked_col(input_key_col, input.input_table.activator_tracked_poly());
    ...
    output_key_col =
        apply_activator_to_tracked_col(output_key_col, input.output_table.activator_tracked_poly());
...
fn lex_sort_prove<F: PrimeField, MvPCS: PCS<F, Poly = MLE<F>>, UvPCS: PCS<F, Poly = LDE<F>>>(
    prover: &mut Prover<F, MvPCS, UvPCS>,
    input: &SortPIOPProverInput<F, MvPCS, UvPCS>,
) -> SnarkResult<()> {
    ...
    for sort_tracked_col in &input.output_sort_exprs {
    ...
    let ascending: Vec<bool> = input.output_sort_exprs.iter().map(|col| col.asc).collect();
...
fn perm_verify<F: PrimeField, MvPCS: PCS<F, Poly = MLE<F>>, UvPCS: PCS<F, Poly = LDE<F>>>(
    verifier: &mut Verifier<F, MvPCS, UvPCS>,
    input: &SortPIOPVerifierInput<F, MvPCS, UvPCS>,
) -> SnarkResult<()> {
    assert_eq!(
        input.input_sort_exprs.len(),
        input.output_sort_exprs.len(),
        "sort expressions mismatch between input and output",
    );
    let num_table_cols = input.input_table.num_data_tracked_col_oracles();
    assert!(
        num_table_cols > 0,
    ...
    assert_eq!(
        num_table_cols,
        input.output_table.num_data_tracked_col_oracles(),
...
fn lex_sort_verify<F: PrimeField, MvPCS: PCS<F, Poly = MLE<F>>, UvPCS: PCS<F, Poly = LDE<F>>>(
    verifier: &mut Verifier<F, MvPCS, UvPCS>,
    input: &SortPIOPVerifierInput<F, MvPCS, UvPCS>,
) -> SnarkResult<()> {
    let mut tracked_columns: IndexMap<
    ...
    for tracked_col in &input.output_sort_exprs {
