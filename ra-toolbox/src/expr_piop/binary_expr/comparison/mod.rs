    ...
    UvPCS: PCS<F, Poly = LDE<F>>,
{
    ...
    fn prove_inner(
        ...
        let selected = &diff * &output_data;

        let selected_col = TrackedCol::new(
            selected.clone(),
            activator.clone(),
            input.left_col.field_ref(),
        );
        ...
        let non_selected_col = TrackedCol::new(diff, gated_activator, input.left_col.field_ref());
