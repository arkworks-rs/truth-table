...
#[derive(Debug, Clone)]
pub struct SortPIOPProverInput {
    pub sort: Sort,
}

#[derive(Debug, Clone)]
pub struct SortPIOPVerifierInput {
    pub sort: Sort,
}
...
{
    type ProverInput = SortPIOPProverInput;
    type ProverOutput = ();
    type VerifierOutput = ();
    type VerifierInput = SortPIOPVerifierInput;
...
impl<F, MvPCS, UvPCS> DeepClone<F, MvPCS, UvPCS> for SortPIOPProverInput
where
    F: PrimeField,
    MvPCS: PCS<F, Poly = MLE<F>>,
    UvPCS: PCS<F, Poly = LDE<F>>,
