...
pub(crate) fn build_prover_proof_tree<B>(
...
{
    let proof_tree: ProverProofTree<B> =
        ProverProofTree::from_lp(df_session_ctx, prover_ctx, &optimized_plan, &NodeId::None);
    proof_tree
}

pub(crate) fn build_verifier_proof_tree<B>(
    df_session_ctx: &SessionContext,
    _unoptimized_plan: LogicalPlan,
    optimized_plan: LogicalPlan,
    verifier_ctx: SharedCtx<B>,
) -> VerifierProofTree<B>
where
    F: ark_ff::PrimeField,
    MvPCS: ark_piop::pcs::PCS<F, Poly = MLE<F>>,
