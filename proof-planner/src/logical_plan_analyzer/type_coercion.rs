...
use datafusion::optimizer::{analyzer::AnalyzerRule, utils::NamePreserver};
...
use datafusion_expr::{
    ...
    not,
    type_coercion::{
        binary::{comparison_coercion, like_coercion},
        functions::{data_types_with_aggregate_udf, data_types_with_scalar_udf},
        is_datetime, is_utf8_or_large_utf8,
        other::{get_coerce_type_for_case_expression, get_coerce_type_for_list},
    },
    utils::merge_schema,
    AggregateUDF, Expr, ExprFunctionExt, ExprSchemable, Join, Limit, LogicalPlan, Operator,
...
pub struct CustomizedTypeCoercion {}

impl CustomizedTypeCoercion {
    pub fn new() -> Self {
        Self {}
    }
}
...
impl AnalyzerRule for CustomizedTypeCoercion {
    fn name(&self) -> &str {
        "customized_type_coercion"
    }
...
fn analyze_internal(
    ...
    // get schema representing all available input fields. This is used for data
    // type resolution only, so order does not matter here
    ...
    let mut expr_rewrite = CustomizedTypeCoercionRewriter::new(&schema);
...
pub struct CustomizedTypeCoercionRewriter<'a> {
    pub(crate) schema: &'a DFSchema,
}

impl<'a> CustomizedTypeCoercionRewriter<'a> {
    /// Create a new [`CustomizedTypeCoercionRewriter`] with a provided schema
    ...
    ///
    /// Refer to [`CustomizedTypeCoercionRewriter::coerce_join`] and
    /// [`CustomizedTypeCoercionRewriter::coerce_union`] for type-coercion
    /// approach.
...
impl TreeNodeRewriter for CustomizedTypeCoercionRewriter<'_> {
    type Node = Expr;
    ...
    fn f_up(&mut self, expr: Expr) -> Result<Transformed<Expr>> {
        match expr {
            ...
            Expr::ScalarFunction(ScalarFunction { func, args }) => {
                let new_expr =
                    coerce_arguments_for_signature_with_scalar_udf(args, self.schema, &func)?;
            ...
            Expr::AggregateFunction(expr::AggregateFunction {
                ...
                let new_expr =
                    coerce_arguments_for_signature_with_aggregate_udf(args, self.schema, &func)?;
            ...
            Expr::WindowFunction(WindowFunction {
                ...
                let args = match &fun {
                    expr::WindowFunctionDefinition::AggregateUDF(udf) => {
                        coerce_arguments_for_signature_with_aggregate_udf(args, self.schema, udf)?
                    },
                    _ => args,
                };
...
fn coerce_case_expression(case: Case, schema: &DFSchema) -> Result<Case> {
    ...
    // END
    //
    // Then all aN (a1, a2, a3) must be converted to a common data type in the first
    // example (case-when expression coercion)
    //
    // All xN (x1, x2) must be converted to a boolean data type in the second
    // example (when-boolean expression coercion)
    //
    // And all bN (b1, b2, b3) must be converted to a common data type in both
    // examples (then-else expression coercion)
    //
    // If any fail to find and cast to a common/specific data type, will return
    // error
    //
    ...
    let case_when_coerce_type = case_type
        ...
        .map(|case_type| {
            let when_types = case
                ...
                .collect::<Result<Vec<_>>>()?;
            let coerced_type = get_coerce_type_for_case_expression(&when_types, Some(case_type));
...
pub fn coerce_union_schema(inputs: &[Arc<LogicalPlan>]) -> Result<DFSchema> {
    ...
    for (i, plan) in inputs.iter().enumerate().skip(1) {
        ...
        for (union_datatype, union_nullable, union_field_map, plan_field) in izip!(
            ...
            let coerced_type = comparison_coercion(union_datatype, plan_field.data_type())
                .ok_or_else(|| {
                    plan_datafusion_err!(
