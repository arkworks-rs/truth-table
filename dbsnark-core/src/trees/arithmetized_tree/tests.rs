...
#[test]
fn serializable_table_from_batches_empty() {
    let table =
        ArithmetizedTree::<F, MvPCS, UvPCS>::serializable_table_from_batches(Vec::new()).unwrap();
...
#[test]
fn serializable_table_from_batches_basic() {
    ...
    let table =
        ArithmetizedTree::<F, MvPCS, UvPCS>::serializable_table_from_batches(vec![batch]).unwrap();
