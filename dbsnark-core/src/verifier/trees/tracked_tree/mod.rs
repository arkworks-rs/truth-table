    ...
    pub fn from_proof_tree(
        ...
        for node in ordered_nodes {
            ...
            let is_table_scan = node
                ...
                .is_some();
            let schema_map: IndexMap<String, (DFSchemaRef, bool)> = node
                .hint_generation_plans(&proof_tree)
                .into_iter()
                .map(|(label, (plan, should_materialize))| {
                    (label, (Arc::clone(plan.schema()), should_materialize))
                })
                .collect();
        ...
        for (node_id, is_table_scan, schema_map) in ordered_infos {
            ...
            for (label, (df_schema, should_materialize)) in schema_map {
                if should_materialize {
                    ...
                    if is_table_scan {
                        ...
                        let base_oracle =
                            shared_ctx.table_oracle(schema_ref).unwrap_or_else(|| {
                    ...
                    let table_oracle =
                        TrackedTableOracle::new(schema_owned, columns, table_log_size);
