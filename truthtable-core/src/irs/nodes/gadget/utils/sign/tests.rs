...
#[test]
fn sign_non_positive_lookup_roundtrip() {
    type Backend = DefaultSnarkBackend;
    ...
    let schema = Schema::new(vec![Field::new("input", DataType::Int8, false)]);
    let field_ref = schema.fields()[0].clone();
    let mut tracked_polys = IndexMap::new();
    tracked_polys.insert(field_ref.clone(), tracked_poly);
    let tracked_table = TrackedTable::new(Some(schema.clone()), tracked_polys, log_size);

    let sign_node = Arc::new(SignNode::<Backend>::new(Sign::NonPositive));
    let root = Arc::new(Node::Gadget(sign_node.clone()));
    let tree = Tree::new_from_root(root.clone());
...
#[test]
fn sign_positive_lookup_roundtrip() {
    type Backend = DefaultSnarkBackend;
    ...
    let sign_node = Arc::new(SignNode::<Backend>::new(Sign::Positive));
    let root = Arc::new(Node::Gadget(sign_node.clone()));
    let tree = Tree::new_from_root(root.clone());
...
#[test]
fn sign_negative_lookup_roundtrip() {
    type Backend = DefaultSnarkBackend;
    ...
    let sign_node = Arc::new(SignNode::<Backend>::new(Sign::Negative));
    let root = Arc::new(Node::Gadget(sign_node.clone()));
    let tree = Tree::new_from_root(root.clone());
