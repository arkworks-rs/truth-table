...
impl<F, MvPCS, UvPCS> ProverNode<F, MvPCS, UvPCS> for ProverMaterializeGadget<F, MvPCS, UvPCS>
where
    F: PrimeField,
    MvPCS: PCS<F, Poly = MLE<F>> + Send + Sync + 'static,
    UvPCS: PCS<F, Poly = LDE<F>> + Send + Sync + 'static,
{
    ...
    fn output_data_frame(&self, _proof_tree: &ProverProofTree<F, MvPCS, UvPCS>) -> DataFrame {
        todo!()
    }
...
#[derive(Clone)]
pub struct VerifierMaterializeGadget<F, MvPCS, UvPCS>
where
    F: PrimeField,
    MvPCS: PCS<F, Poly = MLE<F>> + Send + Sync + 'static,
    UvPCS: PCS<F, Poly = LDE<F>> + Send + Sync + 'static,
...
impl<F, MvPCS, UvPCS> VerifierMaterializeGadget<F, MvPCS, UvPCS>
where
    F: PrimeField,
    MvPCS: PCS<F, Poly = MLE<F>> + Send + Sync + 'static,
    UvPCS: PCS<F, Poly = LDE<F>> + Send + Sync + 'static,
...
impl<F, MvPCS, UvPCS> VerifierNode<F, MvPCS, UvPCS> for VerifierMaterializeGadget<F, MvPCS, UvPCS>
where
    F: PrimeField,
    MvPCS: PCS<F, Poly = MLE<F>> + Send + Sync + 'static,
    UvPCS: PCS<F, Poly = LDE<F>> + Send + Sync + 'static,
{
    ...
    fn output_data_frame(&self, _proof_tree: &VerifierProofTree<F, MvPCS, UvPCS>) -> DataFrame {
        todo!()
    }
