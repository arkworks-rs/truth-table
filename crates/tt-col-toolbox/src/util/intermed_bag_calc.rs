...
use crate::PCS;
use crate::tracker_core::prelude::ProverTrackerRef;
use crate::tracker_core::prelude::Col;
use crate::tracker_core::prelude::PolyIOPErrors;
use ark_std::Zero;
use ark_std::One;
use ark_poly::MLE;
use std::cmp::max;
use crate::tracker_core::prelude::TrackedPoly;
    ...
    PCS: PCS<F>,
{
    ...
    let l_mle = MLE::from_evaluations_vec(l_nv, l_evals);
    let l_sel_mle = MLE::from_evaluations_vec(l_nv, l_sel_evals);
    let ma_mle = MLE::from_evaluations_vec(ma_nv, ma_evals);
    let ma_sel_mle = MLE::from_evaluations_vec(ma_nv, ma_sel_evals);
    let mb_mle = MLE::from_evaluations_vec(mb_nv, mb_evals);
    let mb_sel_mle = MLE::from_evaluations_vec(mb_nv, mb_sel_evals);
    let r_mle = MLE::from_evaluations_vec(r_nv, r_evals);
    let r_sel_mle = MLE::from_evaluations_vec(r_nv, r_sel_evals);
    let l_col = Col::new(prover.track_and_commit_poly(l_mle)?, prover.track_and_commit_poly(l_sel_mle)?);
    let ma_col = Col::new(prover.track_and_commit_poly(ma_mle)?, prover.track_and_commit_poly(ma_sel_mle)?);
    let mb_col = Col::new(prover.track_and_commit_poly(mb_mle)?, prover.track_and_commit_poly(mb_sel_mle)?);
    let r_col = Col::new(prover.track_and_commit_poly(r_mle)?, prover.track_and_commit_poly(r_sel_mle)?);
...
where
    ...
    PCS: PCS<F>,
{
    let (l_col, ma_col, mb_col, r_col) = col_lmr_split(prover, col_a, col_b)?;
    ...
    PCS: PCS<F>,
{
    ...
    // create the intermediate cols
    let l_mle = MLE::from_evaluations_vec(left_nv, l_evals);
    let l_sel_mle = MLE::from_evaluations_vec(left_nv, l_sel_evals);
    let l_mul_mle = MLE::from_evaluations_vec(left_nv, l_mul_evals);
    let m_mle = MLE::from_evaluations_vec(mid_nv, m_evals);
    let m_sel_mle = MLE::from_evaluations_vec(mid_nv, m_sel_evals);
    let m_amul_mle = MLE::from_evaluations_vec(mid_nv, m_amul_evals);
    let m_bmul_mle = MLE::from_evaluations_vec(mid_nv, m_bmul_evals);
    let r_mle = MLE::from_evaluations_vec(right_nv, r_evals);
    let r_sel_mle = MLE::from_evaluations_vec(right_nv, r_sel_evals);
    let r_mul_mle = MLE::from_evaluations_vec(right_nv, r_mul_evals);

    let l_col = Col::new(prover.track_and_commit_poly(l_mle)?, prover.track_and_commit_poly(l_sel_mle)?);
    let m_col = Col::new(prover.track_and_commit_poly(m_mle)?, prover.track_and_commit_poly(m_sel_mle)?);
    let r_col = Col::new(prover.track_and_commit_poly(r_mle)?, prover.track_and_commit_poly(r_sel_mle)?);
    let l_mul = prover.track_and_commit_poly(l_mul_mle)?;
    let ma_mul = prover.track_and_commit_poly(m_amul_mle)?;
    let mb_mul = prover.track_and_commit_poly(m_bmul_mle)?;
    let r_mul = prover.track_and_commit_poly(r_mul_mle)?;
...
mod test {
    ...
    pub fn test_col_lmr_split() -> Result<(), PolyIOPErrors> {
        ...
        let srs = PST13::<Bls12_381>::gen_srs_for_testing(&mut rng, 10)?;
        let (pcs_prover_param, _) = PST13::<Bls12_381>::trim(&srs, None, Some(10))?;
        ...
        let a_mle = MLE::from_evaluations_vec(a_nv, a_nums.iter().map(|x| Fr::from(*x as u64)).collect());
        let a_sel_mle = MLE::from_evaluations_vec(a_nv, a_sel_nums.iter().map(|x| Fr::from(*x as u64)).collect());
        let b_mle = MLE::from_evaluations_vec(b_nv, b_nums.iter().map(|x| Fr::from(*x as u64)).collect());
        let b_sel_mle = MLE::from_evaluations_vec(b_nv, b_sel_nums.iter().map(|x| Fr::from(*x as u64)).collect());
        let a_poly = prover.track_and_commit_poly(a_mle)?;
        let a_sel_poly = prover.track_and_commit_poly(a_sel_mle)?;
        let b_poly = prover.track_and_commit_poly(b_mle)?;
        let b_sel_poly = prover.track_and_commit_poly(b_sel_mle)?;
        let a_col = Col::new(a_poly, a_sel_poly);
        let b_col = Col::new(b_poly, b_sel_poly);

        let (l_col, ma_col, mb_col, r_col) = col_lmr_split(&mut prover, &a_col, &b_col)?;
    ...
    fn test_set_lmr_split() -> Result<(), PolyIOPErrors> {
        ...
        let a_col = Col::new(a_poly, a_sel_poly);
        let b_col = Col::new(b_poly, b_sel_poly);
        let (l_col, m_col, r_col) = set_lmr_split(&mut prover, &a_col, &b_col)?;
