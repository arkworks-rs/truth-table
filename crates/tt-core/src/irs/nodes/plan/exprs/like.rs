...
use arithmetic::{
    ...
    encoding::{
        self, EncodedSegment, SideColData, STRING_BND_SUFFIX, STRING_CHARS_SUFFIX,
        STRING_INT_IND_SUFFIX, STRING_LENGTH_SUFFIX, STRING_ORIG_IND_SUFFIX,
    },
    is_system_column,
    table::TrackedTable,
...
impl<B: SnarkBackend> ExprNode<B> {
    ...
    fn find_string_column_base_name_side_prover(
    ...
    fn find_string_column_base_name_side_verifier(
...
impl<B: SnarkBackend> ExprNode<B> {
    ...
    fn scan_and_compute_witness_prover(
        ...
        let (_length_field, length_poly) = scope_table
            ...
            .find(|(f, _)| {
                f.name().as_str()
                    == format!("{base_name}{STRING_LENGTH_SUFFIX}").as_str()
            })
    ...
    fn commit_and_wire_prover(
        ...
        let char_act_new =
            commit_prover(&tracker_rc, char_domain, witness.char_act_new.clone())?;
        ...
        for fw in witness.per_factor.iter() {
            ...
            let leftmost_mask =
                commit_prover(&tracker_rc, char_domain, fw.leftmost_mask.clone())?;
    ...
    fn track_and_wire_verifier(
        ...
        let mut per_factor_oracles: Vec<FactorOracles<B>> =
        ...
        }
        {
            ...
            let ind_oracle = scope_table
                ...
                .map(|(_, p)| p.clone())
                .unwrap_or_else(|| {
                    track_next_verifier(&tracker_rc).expect("track ind fallback")
                });
...
fn build_virtual_char_domain_hint<F: PrimeField>(
    ...
    let row_arr: ArrayRef = Arc::new(Int64Array::from_iter_values(
...
fn build_virtual_char_domain_hint_schema_only(
    name: &str,
) -> DataFusionResult<HintDF> {
    let data_field = Field::new(name, DataType::Int64, false);
    let row_id_field = Field::new(ROW_ID_COL_NAME, DataType::Int64, false);
...
impl<B: SnarkBackend> ExprNode<B> {
    ...
    fn scan_string_side_data_plan_time_prover(
        ...
        let orig_ind: Vec<B::F> =
            orig_ind_u32.iter().map(|v| B::F::from(*v as u64)).collect();
...
impl<B: SnarkBackend> ProverNodeOps<B> for ExprNode<B> {
    ...
    fn initialize_gadget_plans(
        ...
        for (j, fw) in witness.per_factor.iter().enumerate() {
            let mark_hint =
                build_virtual_char_domain_hint::<B::F>("mark", fw.mark.clone()).map_err(|e| {
                    SnarkError::Artifact(format!("like plan hint: mark[{j}]: {e}"))
                })?;
...
impl<B: SnarkBackend> VerifierNodeOps<B> for ExprNode<B> {
    ...
    fn initialize_gadget_plans(
        ...
        for j in 0..self.factors.len() {
            let mark_hint = build_virtual_char_domain_hint_schema_only("mark").map_err(|e| {
                SnarkError::Artifact(format!("like plan hint: mark[{j}]: {e}"))
            })?;
            mcpm_payload.insert(format!("__like_f{j}_mark_plan_hint__"), mark_hint);
        }
