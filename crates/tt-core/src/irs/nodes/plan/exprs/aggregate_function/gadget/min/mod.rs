...
use crate::irs::nodes::utils::{leq, lookup};
...
impl<B: SnarkBackend> ProverNodeOps<B> for GadgetNode<B> {
    ...
    fn initialize_gadgets(
        ...
        let input_groups = payload
            .get(INPUT_RLC_LABEL)
            .cloned()
            .unwrap_or_else(|| panic!("Min Aggregate Function missing input group payload"));
        let output_groups = payload
            .get(OUTPUT_RLC_LABEL)
            .cloned()
            .unwrap_or_else(|| panic!("Min Aggregate Function missing output group payload"));
        ...
        leq_payload.insert(leq::RIGHT_LABEL.to_string(), input_0.clone());
