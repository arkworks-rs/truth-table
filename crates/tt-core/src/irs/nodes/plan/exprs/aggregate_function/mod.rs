...
impl<B: SnarkBackend> ProverNodeOps<B> for ExprNode<B> {
    fn initialize_gadget_plans(
        ...
        gadget_payload.insert(
            crate::irs::nodes::plan::exprs::aggregate_function::gadget::OUTPUT_LABEL.to_string(),
            output_hint_df,
        );
    ...
    fn add_virtual_witness(
        ...
        if self.aggregate_function.func.name() == "count" {
            ...
            if let Some(lookup_node) = lookup_child_from_aggregate_parent(&parent_node)
                ...
                && let Some(multiplicities_table) =
                    lookup_payload.get(crate::irs::nodes::utils::lookup::SUPER_MULTIPLICITIES_LABEL)
            {
    ...
    fn initialize_gadgets(
        ...
        let (orig_rlc, super_rlc) =
            if let Some(supp_node) = supp_child_from_aggregate_parent(&parent_node) {
                ...
                (orig_rlc, super_rlc)
            } else if matches!(self.aggregate_function.func.name(), "sum" | "max" | "min") {
                // Ungrouped aggregate gadgets still compare an input-domain selector
                // with an output-domain selector, even though both keys are constant.
                let input_table = self
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "{} AggregateFunction expects an input table payload",
                            self.aggregate_function.func.name()
                        )
                    });
                (
        ...
        if let Some(gadget) = &self.gadget {
            ...
            for (idx, arg_node) in self.args.iter().enumerate() {
                ...
                gadget_payload.insert(
                    crate::irs::nodes::plan::exprs::aggregate_function::gadget::input_label(idx),
                    arg_table,
                );
            }

            gadget_payload.insert(
                crate::irs::nodes::plan::exprs::aggregate_function::gadget::OUTPUT_LABEL
                    .to_string(),
                aggr_expr_table,
            );
            gadget_payload.insert(
        ...
        let (orig_rlc, super_rlc) =
            if let Some(supp_node) = supp_child_from_aggregate_parent(&parent_node) {
                ...
                let input_table = self
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "{} AggregateFunction expects an input oracle table payload",
                            self.aggregate_function.func.name()
                        )
                    });
                (
