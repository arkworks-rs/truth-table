...
use crate::{
    irs::{
        nodes::{IsGadgetNode, IsNode, Node, ProverNodeOps, VerifierNodeOps, utils::sign},
...
impl<B: SnarkBackend> ProverNodeOps<B> for GadgetNode<B> {
    ...
    fn initialize_gadgets(
        ...
        let (Some(left_input), Some(right_input)) = (
            ...
            panic!("Expected left and right inputs for geq gadget");
        };

        debug_assert_eq!(
            left_input.data_tracked_polys_indices().len(),
            1,
            "Geq gadget supports one tracked polynomial per input."
        );
        debug_assert_eq!(
        ...
        debug_assert_eq!(
            ...
            "Geq gadget inputs must share the same activator polynomial"
        );
        ...
        let data_field = left_input
            ...
            .cloned()
            .expect("Geq left input should include a data column");
...
impl<B: SnarkBackend> VerifierNodeOps<B> for GadgetNode<B> {
    ...
    fn initialize_gadgets(
        ...
        debug_assert_eq!(
            ...
            1,
            "Geq gadget supports one tracked oracle per input."
        );
        debug_assert_eq!(
        ...
        debug_assert_eq!(
            left_input.activator_tracked_poly(),
            right_input.activator_tracked_poly(),
            "Geq gadget inputs must share the same activator oracle"
        );
