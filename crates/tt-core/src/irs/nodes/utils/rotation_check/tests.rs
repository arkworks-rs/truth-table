...
fn build_gadget(shift: usize, direction: Direction) -> Arc<Node<B>> {
    Arc::new(Node::Gadget(Arc::new(GadgetNode::<B>::new(shift, direction))))
}
...
fn right_shift_by_one() {
    let left: Vec<F> = vec![F::from(10u64), F::from(20u64), F::from(30u64), F::from(40u64)];
    // right[i] = left[(i - 1) mod 4] = [40, 10, 20, 30]
    let right = rotate_right(&left, 1);
    assert_eq!(
        right,
        vec![F::from(40u64), F::from(10u64), F::from(20u64), F::from(30u64)]
    );
...
fn left_shift_by_one() {
    ...
    assert_eq!(
        right,
        vec![F::from(20u64), F::from(30u64), F::from(40u64), F::from(10u64)]
    );
...
fn arbitrary_permutation_rejected() {
    ...
    let left: Vec<F> = vec![F::from(10u64), F::from(20u64), F::from(30u64), F::from(40u64)];
    let right = vec![F::from(20u64), F::from(10u64), F::from(40u64), F::from(30u64)];
    assert!(
        run(1, Direction::Right, left, right).is_err(),
