...
impl<B: SnarkBackend> IsGadgetNode<B> for GadgetNode<B> {
    fn prove(
        ...
        match non_zero_poly.as_constant() {
            None => prover.add_mv_nozerocheck_claim(non_zero_poly.id())?,
            Some(cnst) => assert!(
                !cnst.is_zero(),
    ...
    fn verify(
        ...
        match non_zero_oracle.as_constant() {
            None => verifier.add_mv_nozerocheck_claim(non_zero_oracle.id()),
            Some(cnst) => assert!(
                !cnst.is_zero(),
                "Non-zero check on zero constant polynomial in neq gadget"
            ),
