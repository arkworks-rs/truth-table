...
pub fn parse_like_pattern<F: ark_ff::PrimeField>(
    ...
    while i < bytes.len() {
        if bytes[i] == b'_' {
            return Err(
                "LIKE `_` (single-char wildcard) is not supported yet".to_string(),
            );
        }
...
impl<B: SnarkBackend> GadgetNode<B> {
    ...
    pub fn new_with_nodup_mode(
        ...
        assert!(!factors.is_empty(), "SweepFactors: factor list must be non-empty");
        for (pat, _) in &factors {
            assert!(!pat.is_empty(), "SweepFactors: each factor pattern must be non-empty");
        }
        let t = factors.len();
...
impl<B: SnarkBackend> IsNode<B> for GadgetNode<B> {
    ...
    fn children(&self) -> Vec<Arc<Node<B>>> {
        ...
        for (fp, chain) in self.factor_placements.iter().zip(self.rotation_chains.iter()) {
...
fn rebuild_char_input_prover<B: SnarkBackend>(
    ...
    let char_col = char_input.tracked_col_by_ind(data_indices[0]).data_tracked_poly();
    let orig_ind = char_input.tracked_col_by_ind(data_indices[1]).data_tracked_poly();
    let int_ind = char_input.tracked_col_by_ind(data_indices[2]).data_tracked_poly();
    let bnd = char_input.tracked_col_by_ind(data_indices[3]).data_tracked_poly();
...
fn rebuild_str_input_prover<B: SnarkBackend>(
    ...
    assert!(!data_indices.is_empty(), "SweepFactors: STR_INPUT must have at least ind");
    let ind = str_input.tracked_col_by_ind(data_indices[0]).data_tracked_poly();
...
fn set_factor_placement_payload_prover<B: SnarkBackend>(
    ...
    let rotated =
        TrackedTable::new(Some(rot_schema), rot_polys, char_domain);
    ...
    child.insert(factor_placement::OCCURS_LABEL.to_string(), factor.occurs.clone());
    child.insert(factor_placement::MATCH_LABEL.to_string(), factor.match_str.clone());
    child.insert(factor_placement::MARK_LABEL.to_string(), factor.mark.clone());
    child.insert(factor_placement::START_LABEL.to_string(), factor.start.clone());
    child.insert(
...
fn set_factor_placement_payload_verifier<B: SnarkBackend>(
    ...
    let rotated =
        TrackedTableOracle::new(Some(rot_schema), rot_oracles, char_domain);
...
impl<B: SnarkBackend> VerifierNodeOps<B> for GadgetNode<B> {
    ...
    fn initialize_gadgets(
        ...
        for j in 0..t {
            ...
            if let Some(past) = factor.past_col.as_ref() {
                set_bool_past_payload_verifier(
                    &self.bool_pasts[j],
                    past,
                    virtualized_ir,
                );
