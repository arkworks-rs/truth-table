...
mod tests {
    ...
    #[test]
    fn aligns_comparison_literal_with_decimal_column() -> Result<()> {
        let schema = Schema::new(vec![Field::new("a", DataType::Decimal128(18, 3), true)]);
        let plan = table_scan(Some("t"), &schema, None)?
            .filter(binary_expr(
                col("a"),
                Operator::LtEq,
                Expr::Literal(ScalarValue::Int64(Some(100000))),
            ))?
            .build()?;
        ...
        let Expr::BinaryExpr(binary) = &filter.predicate else {
            panic!(
                "expected binary expr predicate, found {:?}",
                filter.predicate
            );
        };
        ...
        match binary.right.as_ref() {
            ...
            other => panic!("expected cast on comparison literal, found {other:?}"),
        }
