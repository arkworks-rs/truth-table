...
async fn witness_plan_executes_all_witness_queries() {
    ...
    let witness_plan = WitnessPlan::from_proof_plan(&ctx, Arc::clone(&proof_plan))
        .await
        .unwrap();
    let witness_dot = DisplayableWitnessPlan::new(&proof_plan, &witness_plan).graphviz();
    println!("WitnessPlan DOT:\n{}", witness_dot);

    // For every proof-plan node ensure we collected batches for each declared plan.
    let proof_nodes = crate::ra_proof_plan::sorted_descendants(Arc::clone(&proof_plan));
