...
/// A serializable representation of an [`ArithTable`], where the tracked
/// polynomials are materialized so the table can be persisted or transmitted
/// without the original prover tracker state.
pub struct SerializableArithTable<F: PrimeField> {
    schema: Option<Schema>,
    data_polys: Vec<(FieldRef, MLE<F>)>,
    size: usize,
}

impl<F: PrimeField> SerializableArithTable<F> {
    ...
    pub fn from_arith_table<MvPCS, UvPCS>(table: &ArithTable<F, MvPCS, UvPCS>) -> Self
    where
        MvPCS: PCS<F, Poly = MLE<F>>,
        UvPCS: PCS<F, Poly = LDE<F>>,
    {
...
impl<F: PrimeField, MvPCS, UvPCS> From<ArithTable<F, MvPCS, UvPCS>> for SerializableArithTable<F>
where
    MvPCS: PCS<F, Poly = MLE<F>>,
    UvPCS: PCS<F, Poly = LDE<F>>,
{
    fn from(table: ArithTable<F, MvPCS, UvPCS>) -> Self {
        Self::from_arith_table(&table)
    }
}

impl<F: PrimeField> CanonicalSerialize for SerializableArithTable<F> {
...
impl<F: PrimeField> CanonicalDeserialize for SerializableArithTable<F> {
...
impl<F: PrimeField> Valid for SerializableArithTable<F> {
